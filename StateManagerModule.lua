...
local function useState(state,name,value,domain, waitFor, isGet)
	if isGet then
	...
	for i,v in ipairs(state.values[name].functions) do
...
function StateManager.initDomain(props)
	...
	function state:define(name, func)
		if not props.name then
			warn('No name prop provided in StateManager.create => ', props)
			return
		end
		if self.values[name] then
			table.insert(self.values[name].functions, func)
		else
			self.values[name] = {
				functions = {
					func
	...
	binder.Parent = dir
	binder.OnInvoke = function(name, value, waitFor, isGet)
		return useState(state,name,value,props.name, waitFor, isGet)
	end
	...
	if props.remote and isClient then
		...
		if not remote then
			...
			if not myRemote then
				...
				myRemote.OnClientInvoke = function(name, value, waitFor, isGet)
					return useState(state,name,value,props.name, waitFor, isGet)
				end
			end
		end
	elseif props.remote and not isClient then
		...
		remote.OnServerInvoke = function(player, name, value, waitFor, isGet)
			return useState(state,name,value,props.name, waitFor, isGet)
		end
	end
